...
def run(args):
    ...
    while True:
        ...
        #     lines.append(line)
        text = '''
        Hard negative mining is a technique used in machine learning, particularly in the context of training models for object detection, facial recognition, and other tasks that involve classification or detection. The goal of hard negative mining is to improve the model's accuracy and robustness by focusing on difficult examples that are currently misclassified by the model. Here’s how it works:

        1. Training the Model Initially
        First, you train your model on a given dataset using standard training procedures.
        2. Identifying Hard Negatives
        After training, you evaluate the model on a set of training or validation data.
        Hard negatives are those examples that are negative (i.e., they do not contain the object or feature of interest) but are misclassified as positive by the model. These are "hard" because the model finds them difficult to correctly classify.
        '''
        if len(text) == 0:
            break
        # evaluate text
